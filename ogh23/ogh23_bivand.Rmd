...
Since the publication of the second edition of ASDAR (https://asdar-book.org/) ten years ago [@asdar], and based on what we had learned about representing spatial data, it became clear that alternatives to `sp` should be sought, leading to `sf`, and most recently to the Spatial Data Science book [@sdsr]. In parallel, Robert Hijmans started moving more of `raster` processing into compiled code, leading to the development of the `terra` package no longer using `rgdal` or `rgeos`. So modernisation has been taking place in infrastructure packages.
...
So far four project reports have been written (https://r-spatial.github.io/evolution/), and a presentation (with [video](https://www.youtube.com/watch?v=TlpjIqTPMCA)) given at the Center for Spatial Data Science at the University of Chicago in January 2023. 
